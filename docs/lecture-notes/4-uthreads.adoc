...
* If `next_thread` already ran in the past, then it execution was suspended due to a `context_switch` execution (where it was the running thread) and therefore its context is saved on its own stack, exactly as shown on <<img-uthreads-curr-thread-stack, Figure {counter:img-uthreads-curr-thread-stack}>>
